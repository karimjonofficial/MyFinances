...
fun OrderItemApiModel.toModel(formatDecimal: FormatDecimal): OrderItemModel {
    return OrderItemModel(
        name = "Product ${product.productTitle}", // Assuming you want title name but have ID
        amount = formatDecimal.formatDecimal(amount.toDouble())
    )
}

fun Order.map(formatDate: FormatDate, formatTime: FormatTime, formatPrice: FormatPrice): OrderCardModel {
    return OrderCardModel(
        title = "${client.firstName} ${client.lastName ?: ""}",
